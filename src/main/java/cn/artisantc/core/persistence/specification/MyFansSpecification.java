 ...
 * @since 1.0
 */
public class MyFansSpecification {

    public static Specification<MyFans> findAllByMomentId(final String mobile) {
        return new Specification<MyFans>() {
            @Override
            public Predicate toPredicate(Root<MyFans> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
                List<Predicate> predicateList = new ArrayList<>();// 存放多个条件
                predicateList.add(cb.equal(root.get("i").get("mobile"), mobile));
