...
<head>
    ...
    <title><fmt:message key="text.page.information"/> - <fmt:message key="text.page.head.title"/></title>
...
<div class="container" role="main">
    ...
    <div class="col-sm-11 col-md-11 col-lg-11 page-header">
        <h1><fmt:message key="text.page.information"/>
...
<script>
    $(function () {
        ...
        // “新增资讯”模态框内容“加载成功”回调函数
        var createContent, ajaxData, imageName, imageWidth, imageHeight;
...
//                        response = data.response, reader = data.reader;
            ...
            $("#save-btn").click(
                function () {
                    saveInformation("/administrator/information");
                }
            );
            ...
            $("#save-and-publish-btn").click(
                function () {
                    saveInformation("/administrator/information/publish");
                }
            );
            ...
            function saveInformation(url) {
                ...
                if (undefined == imageName || null == imageName || "" === imageName.trim()) {
                    ...
                    createContent = "{\"imageName\":\"" + imageName + "\",\"imageWidth\":\"" + imageWidth + "\",\"imageHeight\":\"" + imageHeight + "\"}";// 封面图片的数据，JSON格式
                    ajaxData = $("#informationForm").serializeJSON();// 表单数据，JSON格式
                    $.extend(ajaxData, jQuery.parseJSON(createContent));// 合并数据，JSON格式
    ...
    /* 分页列表功能 */
    var pageHref = "/administrator/information";// 分页数据的URL
    ...
    function getListSuccessCallback(data, textStatus, jqXHR) {
        ...
        $.each(data.information, function () {
            ...
            if (null != this.coverUrl && this.coverUrl != "") {
                informationImg = "<img src=\"" + this.coverUrl + "\" data-src=\"holder.js/100%x100%\" alt=\"" + this.title + "\">";
            }
            var informationTitle = this.title;
            if (informationTitle.length > 8) {
                informationTitle = informationTitle.substring(0, 8) + "...";
            }

            var $appendData = $("<div class=\"thumbnail img-cover\">" +
                "<a href=\"<c:url value="/console/information/"/>" + this.informationId + "\" target=\"_blank\" title=\"" + this.title + "\">" +
                informationImg +
                "<div class=\"caption\">" +
                "<h4>" + informationTitle + "</h4>" +
                "<p>" + this.source + "<span class=\"pull-right\">浏览" + this.browseTimes + "次</span></p>" +
                "<p>" + this.status + "<span class=\"pull-right\">" + this.createDateTime + "</span></p>" +
                "</div>" +
                "</a>" +
                "</div>");
