...
<head>
    ...
    <title><fmt:message key="text.page.real.name.status.pending.review"/> - <fmt:message key="text.page.head.title"/></title>
...
<div class="container" role="main">
    ...
    <div class="page-header">
        <h1><fmt:message key="text.page.real.name.status.pending.review"/>
...
<script>
    ...
    /* 分页列表功能 */
    var pageHref = "<c:url value='/administrator/real-name/pending-review' />";
    ...
    function getListSuccessCallback(data, textStatus, jqXHR) {
        ...
        $.each(data.realNames, function () {
            var realNameImages = this.images;
            var realNameImg = "";
            if (realNameImages != undefined && realNameImages != null) {
                $.each(this.images, function () {
                    if (null != this.imageUrl && this.imageUrl != "") {
                        realNameImg = realNameImg + "<img class=\"img-thumbnail\" src=\"" + this.imageUrl + "\" data-src=\"holder.js/100%x100%\">";
                    }
                });
            }

            var realNameTitle = this.realName;
            if (realNameTitle.length > 8) {
                realNameTitle = realNameTitle.substring(0, 8) + "...";
            }

            var $appendData = $("<div class=\"thumbnail img-cover\">" +
                    "<a href=\"<c:url value="/console/real-name/"/>" + this.realNameId + "\" target=\"_blank\">" +
                    realNameImg +
                    "<div class=\"caption\">" +
                    "<h4>" + realNameTitle + "</h4>" + " <span class=\"pull-right\">" + this.createDateTime + "</span></p>" +
                    "</div>" +
                    "</a>" +
                    "</div>");
