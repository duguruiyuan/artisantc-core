...
<head>
    ...
    <title><fmt:message key="text.page.items.status.pending.review"/> - <fmt:message key="text.page.head.title"/></title>
...
<div class="container" role="main">
    ...
    <div class="page-header">
        <h1><fmt:message key="text.page.items.status.pending.review"/>
...
<script>
    ...
    /* 分页列表功能 */
    var pageHref = "<c:url value='/administrator/items/pending-review' />";
    ...
    function getListSuccessCallback(data, textStatus, jqXHR) {
        ...
        $.each(data.items, function () {
            ...
            if (null != this.coverUrl && this.coverUrl != "") {
                itemImg = "<img src=\"" + this.coverUrl + "\" data-src=\"holder.js/100%x100%\" alt=\"" + this.title + "\" title=\"" + this.title + "\">";
            }
            var itemTitle = this.title;
            if (itemTitle.length > 8) {
                itemTitle = itemTitle.substring(0, 8) + "...";
            }

            var $appendData = $("<div class=\"thumbnail img-cover\">" +
                    "<a href=\"<c:url value="/console/items/"/>" + this.itemId + "\" target=\"_blank\">" +
                    itemImg +
                    "<div class=\"caption\">" +
                    "<h4>" + itemTitle + "</h4>" +
                    "<p>￥" + this.initialPrice + " 起拍价<span class=\"pull-right\">" + this.nickname + "</span></p>" +
                    "</div>" +
                    "</a>" +
                    "</div>");
