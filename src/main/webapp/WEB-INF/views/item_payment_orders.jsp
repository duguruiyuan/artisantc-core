...
<head>
    ...
    <title><fmt:message key="text.page.item.payment.orders"/> - <fmt:message key="text.page.head.title"/></title>
...
<div class="container" role="main">
    ...
    <div class="col-sm-11 col-md-11 col-lg-11 page-header">
        <h1><fmt:message key="text.page.item.payment.orders"/>
...
<script>
    ...
    /* 分页列表功能 */
    var pageHref = "<c:url value='/administrator/item-payment-orders' />";
    ...
    function getListSuccessCallback(data, textStatus, jqXHR) {
        ...
        $.each(data.itemPaymentOrders, function () {
            var itemImg = "";
            if (null != this.coverUrl && this.coverUrl != "") {
                itemImg = "<img src=\"" + this.coverUrl + "\" data-src=\"holder.js/100%x100%\" alt=\"" + this.title + "\">";
            }
            var $appendData = $("<div class=\"thumbnail img-cover\">" +
                    "<a href=\"<c:url value="/console/item-payment-orders/"/>" + this.itemOrderNumber + "\" target=\"_blank\">" +
                    itemImg +
                    "<div class=\"caption\">" +
                    "<p>" + this.title + "</p>" +
                    "<p>" + this.paymentOrderStatus + "<span class=\"pull-right\">" + this.paymentOrderCreateDateTime + "</span></p>" +
                    "</div>" +
                    "</a>" +
                    "</div>");
